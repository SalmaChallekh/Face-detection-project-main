...
# Revenir à la forme originale des images
X_restored = pca.inverse_transform(X_pca)

# Extraire un échantillon de 10 images
sample_indices = np.random.choice(X_restored.shape[0], 10, replace=False)
sample_images_restored = X_restored[sample_indices, :]

# Afficher les images
plt.figure(figsize=(15, 6))
for i in range(10):
    plt.subplot(2, 5, i + 1)
    plt.imshow(sample_images_restored[i].reshape(images_normalized.shape[1], images_normalized.shape[2]), cmap='gray')
    plt.title(f"Image {i + 1}")
    plt.axis('off')
plt.show()
